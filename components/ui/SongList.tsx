...
export const SongList: React.FC<SongListProps> = ({ songs, playlistId, navigateToArtist, onInteraction }) => {
  const { playSong, addSongNext, addSongsToEnd } = useContext(PlayerContext);
  const { isFavoriteSong, toggleFavoriteSong, removeSongFromPlaylist } = useContext(UserMusicContext);
  ...
  if (!songs || songs.length === 0) {
    return <p className="text-gray-400">No songs to display.</p>;
  }
      ...
      {songs.map((song, index) => {
        ...
        return (
          <div
            ...
            className="group flex items-center p-2 rounded-lg hover:bg-white/10 transition-colors duration-200 h-20 cursor-pointer"
          >
            <div className="w-8 text-center text-gray-400 mr-4 flex-shrink-0">{index + 1}</div>
            <img src={imageUrl} alt={song.name} className="w-12 h-12 rounded-md mr-4 flex-shrink-0" />
            <div className="flex-1 min-w-0">
              <p className="font-semibold text-white leading-snug line-clamp-2" title={song.name}>{song.name}</p>
            ...
            <div className="flex items-center space-x-2 ml-4 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                <button 
                    ...
                    className="text-gray-400 hover:text-[#fc4b08] p-2 rounded-full hover:bg-white/10">
                    <HeartIcon className={`w-5 h-5 ${isFavoriteSong(song.id) ? 'fill-[#fc4b08] text-[#fc4b08]' : ''}`} />
                </button>
                {playlistId && (
                    <button 
                        onClick={(e) => { e.stopPropagation(); removeSongFromPlaylist(playlistId, song.id); }} 
                        className="text-gray-400 hover:text-red-500 p-2 rounded-full hover:bg-white/10">
                        <TrashIcon className="w-5 h-5" />
                    </button>
                )}
                <div className="relative" ref={activeMenu === song.id ? menuRef : null}>
                    <button onClick={(e) => { e.stopPropagation(); setActiveMenu(activeMenu === song.id ? null : song.id); }} className="text-gray-400 hover:text-white p-2 rounded-full hover:bg-white/10">
            ...
            <button
                ...
                className="ml-4 w-10 h-10 flex items-center justify-center rounded-full bg-[#fc4b08] text-black opacity-0 group-hover:opacity-100 transform group-hover:scale-105 scale-90 transition-all duration-200 flex-shrink-0 shadow-lg shadow-[#fc4b08]/30 hover:bg-[#ff5f22]"
            >
