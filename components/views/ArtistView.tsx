...
const ArtistView: React.FC<ArtistViewProps> = ({ artistId, setActiveView, navigateToAlbum, navigateToArtist }) => {
  ...
  if (!artist) return <div className="p-8 text-center text-gray-400">Artist not found or failed to load.</div>;
  ...
  return (
    <div className="text-white">
      <div className="p-8 flex flex-col sm:flex-row items-center sm:items-end gap-6 relative">
        ...
        {imageUrl && <img src={imageUrl} alt={artist.name} className="w-40 h-40 sm:w-52 sm:h-52 rounded-full shadow-2xl z-10 flex-shrink-0" />}
      ...
      <div className="px-8 pb-8 space-y-12">
        ...
        {artist.similarArtists && artist.similarArtists.length > 0 && (
          <section>
            <h2 className="text-3xl font-bold mb-4">Fans Also Like</h2>
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-6">
